...
function extractLocationFromFilename(filename) {
  // Remove " Training Matrix - Staff Matrix.csv" from end
...
async function completeRebuildFromCSV() {
  console.log('\n' + '═'.repeat(120));
  console.log('  COMPLETE REBUILD FROM CSV - DELETE ALL AND REIMPORT');
  ...
  locations?.forEach(loc => {
    locationMap[loc.name.trim()] = loc.id;  // Trim whitespace
  });
  ...
  for (const csvFile of csvFiles) {
    ...
    try {
      ...
      // Data starts at row 6 (index 5, but skip to index 6 after Staff Name, Notes, Date valid rows)
      ...
      console.log(`  Creating ${recordsToInsert.length} base records...`);
  ...
  console.log(`  • Total inserted: ${totalInserted} base records`);
  console.log(`  • Total updated: ${totalUpdated} records with dates\n`);
  console.log(`  TOTAL RECORDS: ${totalInserted + totalUpdated}\n`);
  console.log('═'.repeat(120) + '\n');
}
