...
async function fullRebuild() {
  console.log('\n' + '═'.repeat(120));
  console.log('  COMPLETE REBUILD: DELETE AND RECREATE ALL RECORDS FROM CSV');
  ...
  for (const csvFile of csvFiles) {
    ...
    try {
      ...
      console.log(`Processing: ${location} (${courseNames.length} courses, ${coursesWithData} with data)\n`);

      // Get current staff for this location
      const { data: staffLocations } = await supabase
        .from('staff_locations')
        .select('staff_id')
        .eq('location_id', locationId);

      const staffIdsForLocation = staffLocations?.map(sl => sl.staff_id) || [];

      // DELETE all existing records for this location
      ...
      console.log(`  ✓ Deleted all existing records for ${location}`);

      // CREATE all records in batches
      const batchSize = 100;
      let inserted = 0;
      let updated = 0;

      // First pass: create base records with N/A status
      const baseBatch = [];
      ...
      // Insert in batches
      for (let i = 0; i < baseBatch.length; i += batchSize) {
        const batch = baseBatch.slice(i, i + batchSize);
      ...
      console.log(`  ✓ Inserted ${inserted} base records`);

      // Second pass: update with real dates from CSV
      for (let i = staffNameRow + 3; i < records.length; i++) {
        const staffName = (records[i][0] || '').trim();
        if (!staffName || !staffMap[staffName]) continue;
      ...
      console.log(`  ✓ Final result: ${inserted + updated} total records for ${location}\n`);
  ...
  console.log(`  • Created: ${globalInserted} base records`);
  console.log(`  • Updated with dates: ${globalUpdated} records\n`);
  console.log('═'.repeat(120) + '\n');
}
