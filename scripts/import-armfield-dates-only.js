...
function parseDate(dateStr) {
  ...
  try {
    ...
    return { date: date.toISOString().split('T')[0], status: 'completed' }; // Return YYYY-MM-DD format
  } catch (error) {
    return { date: null, status: null };
  }
}
...
async function importArmfieldDates() {
  try {
    ...
    const courses = [];
    const courseMap = {}; // Map index to course name
    ...
    for (const course of courses) {
      ...
      if (existing && existing.length > 0) {
        ...
        console.log(`WARNING: Course not found: ${course.name}`);
      }
    }
    ...
    for (let rowIndex = headerRowIndex + 1; rowIndex < records.length; rowIndex++) {
      ...
      if (!staffMembers || staffMembers.length === 0) {
        console.log(`SKIP: Staff member not found: ${staffName.trim()}`);
        skippedCount++;
        continue;
      }
      ...
      for (let colIndex = 1; colIndex < row.length; colIndex++) {
        ...
        if (date || status) {
          try {
            // Use provided date or today's date if only status is provided
            ...
            if (!error) {
              ...
              console.error(`Error saving training for ${staffName.trim()} - ${courseName}:`, error);
            }
          } catch (e) {
            console.error(`Exception saving training for ${staffName.trim()} - ${courseName}:`, e);
          }
        }
      }
    }
    ...
    console.log(`Staff skipped (not found): ${skippedCount}`);
